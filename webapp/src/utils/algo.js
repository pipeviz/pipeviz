...
function reachCounter() {
  ...
  var msfc = _.memoize(function (accum, value) {});
  ...
  return {
    ...
    throughPredecessors: function (g, v, filter) {
      ...
      if (typeof pred === 'undefined' || pred.length === 0) {
        return 0;
      }
...
var vizExtractor = module.exports.vizExtractor = {
  ...
  findGuideCommits: function (pvg, repo, branchlvl, taglvl) {
    ...
    _.each(pvg.vertices(pvd.pq.or(pvd.isType("git-branch"), pvd.isType("git-tag"), pvd.isType("logic-state"))), function (v) {
      var level = V_UNINTERESTING;
      var commit = query.getCommit(pvg, v);

      if (typeof commit !== 'undefined' && commit.propv("repository") === repo) {
        switch (v.Typ()) {
          case "logic-state":
  ...
  focalLogicStateByRepo: function (pvg, repo) {
    var focalCommits = {};
    var focal = _.filter(_.map(pvg.verticesWithType("logic-state"), function (d) { return _.create(pvd.vertexProto, d); }), function (v) {
        var vedges = _.filter(_.map(v.outEdges, function (edgeId) { return pvg.get(edgeId); }), pvd.isType("version"));
        if (vedges.length === 0) {
          return false;
        }
  ...
  focalTransposedGraph: function (cg, focalCommits) {
    // graph with all non-focal vertices contracted and edges transposed (direction reversed)
    var fg = new graphlib.Graph();
    var visited = {};
    // Depth-first walker, builds the focal graph
  ...
  treeAndRoot: function (cg, boundaryCommits, focalCommits) {
    // A tree, almost an induced subgraph, of first-parent paths in the commit graph
    ...
    var root;
    // Tracks "out-of-bound" commits; those between the natural root and the earliest candidate
    var oob = {};
  ...
  extractTree: function (cg, tree, root, focalCommits, festoonedCommits, rankCommits) {
    // metadata we build for each vertex. keyed by vertex id
    var vmeta = {};
    // we can start figuring out some links in the next walk
    var protolinks = [];
    // total number of divergent segment paths. starts at 0, increases as needed
    var segments = 0;
    // maximum depth reached. Useful info later that we can avoid recalculating
    var diameter = 0;
    // list of interesting depths (so, no elision), to avoid another walk later
    var idepths = [];
    // create a new memoizing reach counter
    var rc = reachCounter();
    // just to avoid repeating this in the walk
    var rankIds = _.keys(rankCommits);
    var mainwalk = function (v, path, segment, pseg) {
 ...
 */
module.exports.extractVizGraph = function (pvg, cg, guideCommits, elide) {
  var boundaryCommits = _.pick(guideCommits, function (gc) { return gc.level & V_BOUNDARY; });
  var focalCommits = _.pick(guideCommits, function (gc) { return gc.level & V_FOCAL; });
  var festoonedCommits = _.pick(guideCommits, function (gc) { return gc.level & V_FESTOONED; });
  var rankCommits = _.pick(guideCommits, function (gc) { return gc.level & V_REACHRANK; });
  ...
  var links = [];
  var xmap = {}; // a map of x-positions to labels, for use on a commit axis
 ...
 */
module.exports.createTransforms = function (vpwidth, vpheight, diameter, deviation, revx) {
  // var ar = vpwidth / vpheight;
  var unit = vpwidth / (diameter + 1); // the base grid unit size, presumably in px
...
module.exports.dumpGraph = function (g, rg, mapper) {
  console.log({
...
module.exports.clMapper = function (v) {
  _.assign(v, v.vertex);

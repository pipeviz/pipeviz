...
func Resolve(g CoreGraph, mid uint64, src types.VertexTuple, d types.EdgeSpec) (types.StandardEdge, bool) {
	switch es := d.(type) {
	...
	return types.StandardEdge{}, false
}

func resolveEnvLink(g CoreGraph, mid uint64, src types.VertexTuple, es interpret.EnvLink) (e types.StandardEdge, success bool) {
...
func resolveDataLink(g CoreGraph, mid uint64, src types.VertexTuple, es interpret.DataLink) (e types.StandardEdge, success bool) {
	...
	if !isLocal {
		...
		rv = g.PredecessorsWith(envid, Qbv(types.VType("comm"), "type", "port", "port", es.ConnNet.Port).and(Qbe(types.EType("envlink"))))
		...
		rv = g.PredecessorsWith(sock.ID, Qbe(types.EType("listening"), "proto", es.ConnNet.Proto).and(Qbv(types.VType("process"))))
		if len(rv) != 1 {
		...
		rv = g.PredecessorsWith(envid, Qbv(types.VType("comm"), "path", es.ConnUnix.Path).and(Qbe(types.EType("envlink"))))
		if len(rv) != 1 {
			return
		}
		sock = rv[0]
		...
		// With sock in hand, now find its proc
		rv = g.PredecessorsWith(sock.ID, Qbv(types.VType("process")).and(Qbe(types.EType("listening"))))
		if len(rv) != 1 {
			// TODO could/will we ever allow >1?
			return
		}
	...
	rv = g.SuccessorsWith(rv[0].ID, Qbv(types.VType("parent-dataset")))
	...
	if es.Subset != "" {
		rv = g.PredecessorsWith(rv[0].ID, Qbv(types.VType("dataset"), "name", es.Subset).and(Qbe(types.EType("dataset-hierarchy"))))
		if len(rv) != 1 {
			return
		}
		dataset = rv[0]
...
func resolveSpecCommit(g CoreGraph, mid uint64, src types.VertexTuple, es SpecCommit) (e types.StandardEdge, success bool) {
	e = types.StandardEdge{
		Source: src.ID,
		Props:  ps.NewMap(),
		EType:  "version",
...
func resolveSpecGitCommitParent(g CoreGraph, mid uint64, src types.VertexTuple, es SpecGitCommitParent) (e types.StandardEdge, success bool) {
	e = types.StandardEdge{
		Source: src.ID,
		Props:  ps.NewMap(),
		EType:  "parent-commit",
	...
	re := g.OutWith(src.ID, Qbe(types.EType("parent-commit"), "pnum", es.ParentNum))
	if len(re) > 0 {
		success = true
		e.Target = re[0].Target
		e.ID = re[0].ID
...
func resolveSpecLocalLogic(g CoreGraph, mid uint64, src types.VertexTuple, es SpecLocalLogic) (e types.StandardEdge, success bool) {
	e = types.StandardEdge{
		Source: src.ID,
		Props:  ps.NewMap(),
		EType:  "logic-link",
	...
	re := g.OutWith(src.ID, Qbe(types.EType("logic-link"), "path", es.Path))
	if len(re) == 1 {
	...
	rv := g.PredecessorsWith(envid, Qbv(types.VType("logic-state"), "path", es.Path))
	if len(rv) == 1 {
		success = true
		e.Target = rv[0].ID
	}
...
func resolveNetListener(g CoreGraph, mid uint64, src types.VertexTuple, es SpecNetListener) (e types.StandardEdge, success bool) {
	...
	if hasenv {
		rv := g.PredecessorsWith(envid, Qbv(types.VType("comm"), "type", "port", "port", es.Port))
		if len(rv) == 1 {
			success = true
			e.Target = rv[0].ID
		}
...
func resolveUnixDomainListener(g CoreGraph, mid uint64, src types.VertexTuple, es SpecUnixDomainListener) (e types.StandardEdge, success bool) {
	// check for existing edge; this one is quite straightforward
	...
	envid, _, hasenv := findEnv(g, src)
	if hasenv {
		rv := g.PredecessorsWith(envid, Qbv(types.VType("comm"), "type", "unix", "path", es.Path))
		if len(rv) == 1 {
			success = true
			e.Target = rv[0].ID
		}
...
func resolveSpecDatasetHierarchy(g CoreGraph, mid uint64, src types.VertexTuple, es SpecDatasetHierarchy) (e types.StandardEdge, success bool) {
	e = types.StandardEdge{
		Source: src.ID,
		Props:  ps.NewMap(),
	...
	// no existing link found; search for proc directly
	envid, _, _ := findEnv(g, src)
	rv := g.PredecessorsWith(envid, Qbv(types.VType("parent-dataset"), "name", es.NamePath[0]))
...
func resolveSpecParentDataset(g CoreGraph, mid uint64, src types.VertexTuple, es SpecParentDataset) (e types.StandardEdge, success bool) {
	e = types.StandardEdge{
		Source: src.ID,
		Props:  ps.NewMap(),
	...
	if len(re) == 1 {
		...
		// no existing link found; search for proc directly
		envid, _, _ := findEnv(g, src)
		rv := g.PredecessorsWith(envid, Qbv(types.VType("parent-dataset"), "name", es.Name))
		if len(rv) != 0 { // >1 shouldn't be possible
			success = true
			e.Target = rv[0].ID
		}
...
func resolveDataProvenance(g CoreGraph, mid uint64, src types.VertexTuple, es interpret.DataProvenance) (e types.StandardEdge, success bool) {
	...
	re := g.OutWith(src.ID, Qbe(types.EType("data-provenance")))
	if len(re) == 1 {
...
func resolveDataAlpha(g CoreGraph, mid uint64, src types.VertexTuple, es interpret.DataAlpha) (e types.StandardEdge, success bool) {
...
func findEnv(g CoreGraph, vt types.VertexTuple) (vid int, edge types.StandardEdge, success bool) {
	edge = types.StandardEdge{
		Source: vt.ID,
		Props:  ps.NewMap(),
		EType:  "envlink",
	}
	...
	if vt.ID != 0 {
		re := g.OutWith(vt.ID, Qbe(types.EType("envlink")))
		if len(re) == 1 {
